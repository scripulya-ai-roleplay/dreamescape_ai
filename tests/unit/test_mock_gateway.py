...
class TestMockGateway:
	...
	async def test_generate_response_echoes_message(self, mock_gateway, mock_logger, text):
		"""The mock wraps the incoming message text and reports consistent metadata."""
		result = await mock_gateway.generate_response(_msg(text), history=[])
	...
	@pytest.mark.asyncio
	async def test_generate_response_echoes_chosen_model(self, mock_gateway):
		"""A non-testing model routed to the mock is echoed back."""
		result = await mock_gateway.generate_response(_msg("hi", LLMModelType.gemini_flash_preview), history=[])
		assert result.model == LLMModelType.gemini_flash_preview

	@pytest.mark.asyncio
	async def test_history_does_not_affect_output(self, mock_gateway):
		result = await mock_gateway.generate_response(_msg("hello"), history=[_msg("prior"), _msg("prior 2")])
	...
	async def test_multiple_calls_independent(self, mock_gateway, mock_logger):
		...
		result1 = await mock_gateway.generate_response(_msg("First message"), history=[])
		result2 = await mock_gateway.generate_response(_msg("Second message"), history=[])
