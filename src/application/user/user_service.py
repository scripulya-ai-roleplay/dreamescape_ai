...
class UserService(IUserService):
    ...
    async def find_users_by_dto(self, user_filters_dto: UserDTO) -> Page[User]:
        logger.info(f"Searching users with filters: {user_filters_dto}")
        
        try:
            result = await self._user_gateway.find_users_by_filters(user_filters_dto, user_filters_dto.limit, user_filters_dto.offset)
            logger.info(f"Found {result.count} users")
            return result
    ...
    async def create_user(self, user: User) -> User:
        logger.info(f"Creating user: {user.username or user.test_username}")
        ...
        async with self.uow:
            ...
            if user.google_id:
                existing_users = await self._user_gateway.find_users_by_filters(
                    UserDTO(google_id=[user.google_id]), limit=1, offset=0
                )
                if existing_users.result:
